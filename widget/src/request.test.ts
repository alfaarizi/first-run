...
test("posts with keepalive and the key, timestamp, and signature headers", async () => {
...
test("marks 429 and 5xx retryable and other 4xx permanent", async () => {
  for (const [status, retryable] of [
    ...
    expect(await post(config, "/v1/e", "{}")).toEqual({ ok: false, retryable });
  }
});

test("marks a network failure retryable", async () => {
  vi.stubGlobal("fetch", vi.fn<typeof fetch>().mockRejectedValue(new TypeError("failed")));

  expect(await post(config, "/v1/e", "{}")).toEqual({ ok: false, retryable: true });
});
